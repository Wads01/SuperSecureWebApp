...
<main class="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center gap-6 p-6">
	...
	<form method="POST" class="flex flex-col gap-4 rounded border p-4">
		...
		<div class="rounded border border-gray-200 bg-gray-50 px-3 py-2 text-xs">
			...
			<p class={requirementClass(hasUppercase(password))}>Password should include an uppercase letter.</p>
			<p class={requirementClass(hasLowercase(password))}>Password should include a lowercase letter.</p>
			<p class={requirementClass(hasNumber(password))}>Password should include a number.</p>
			<p class={requirementClass(hasSpecial(password))}>Password should include a special character.</p>
		</div>
